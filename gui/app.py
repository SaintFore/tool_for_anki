...
class TabView(customtkinter.CTkTabview):
    def __init__(self, master, **kwargs):
        ...
        self.tab("添加标签").grid_columnconfigure(0, weight=1)
        self.tab("添加标签").grid_rowconfigure(0, weight=1)

        self.tab("标准化").grid_columnconfigure(0, weight=1)
        self.tab("标准化").grid_rowconfigure(0, weight=1)

        

        # 创建标签提示用户
        self.label = customtkinter.CTkLabel(
            self.tab("标准化"), text="将文件拖放到下方区域"
        )
        self.label.grid(row=0, column=0, padx=20, pady=10)

        # 创建文本框作为拖放区域
        self.drop_area = customtkinter.CTkTextbox(self.tab("标准化"), height=200)
